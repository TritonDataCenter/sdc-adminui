...
Sessions.prototype.get = function(id, cb) {
    var self = this;
    if (false === this.redis.connected) {
        return cb(new Error('No Connection to redis'));
    }
...
Sessions.prototype.touch = function(id, cb) {
    ...
    if (false === this.redis.connected) {
        return cb('No Connection to redis');
    }
    var k = _sessionKey(id);
...
Sessions.prototype.create = function(data, callback) {
    ...
    if (false === this.redis.connected) {
        return callback(new Error('No Connection to redis'));
    }
