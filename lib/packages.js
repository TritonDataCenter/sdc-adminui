...
module.exports.update = function(req, res, next) {
    ...
    pkgclient.get(req.params.uuid, function(err, pkg) {
        ...
        var ptraits = _.clone(pkg.traits);
        var ctraits = _.clone(changes.traits);

        var p = stringify(pkg);
        p.traits = ptraits;

        var c = stringify(changes);
        c.traits = ctraits;

        req.log.info('sdc-clients.package.update',
            {original: p, changes: c}
        );

        pkgclient.update(p, c, function(err, p) {
            if (err) {
                req.log.fatal(err);
                return next(err);
            }
