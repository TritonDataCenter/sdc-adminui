...
function addLimit(req, res, next) {
    var user = req.params.uuid;
    req.ufdsMaster.getLimit(user, req.body.datacenter, function(err, limit) {
        if (err && err.name === 'ResourceNotFound') {
            doAddLimit();
        } else {
            for (var k in limit) {
                req.body[k] = limit[k];
    ...
    function doUpdateLimit() {
        req.log.info('updating limit', {
            user: user,
            limit: req.body
        });
        req.ufdsMaster.updateLimit(user, req.body, function(err, limit) {
            if (err) {
