...
module.exports.list = function listServers(req, res, next) {
    ...
    req.sdc[req.dc].cnapi.listServers(
        ...
        // for (var i = 0; i <= 100; i++) {
        //     var s = _.clone(servers[0]);
        //     s.uuid = require('libuuid').create();
        //     s.hostname = require('crypto').randomBytes(12).toString('hex');
        //     s.headnode = false;
        //     s.setup = false;
        //     s.memory_provisionable_bytes = Math.floor(Math.random()*1024*1024*1024+1)*2;
        ...
        if (req.query.sort === 'current_platform' || req.query.sort === 'boot_platform' || req.query.sort === 'hostname') {
