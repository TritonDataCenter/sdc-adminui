...
ADMINUI.prototype.createServer = function() {
    ...
    server.get('/api/metrics/:name', auth.requireAuth, function getMetrics(req, res, next) {
    ...
    var vms = require('./vms');
    server.get({name: 'listVMs', path: "/api/vms"},
        auth.requireAuth,
        vms.list);

    server.post({name: 'createVM', path: '/api/vms'},
        auth.requireAuth,
    ...
    server.post('/api/users/:uuid/roles', auth.requireAuth, auth.requireRole('operators'), resume, bodyParser, users.addRole);
    server.put('/api/users/:uuid/roles/:role', auth.requireAuth, auth.requireRole('operators'), resume, bodyParser, users.modifyRole);
    server.del('/api/users/:uuid/roles/:role', auth.requireAuth, auth.requireRole('operators'), resume, bodyParser, users.deleteRole);
    ...
    server.get('/api/users/:uuid/policies', auth.requireAuth, users.listPolicies);
    server.get('/api/users/:uuid/policies/:policy', auth.requireAuth, users.getPolicy);
    server.post('/api/users/:uuid/policies', auth.requireAuth, auth.requireRole('operators'), resume, bodyParser, users.addPolicy);
    server.put('/api/users/:uuid/policies/:policy', auth.requireAuth, auth.requireRole('operators'), resume, bodyParser, users.modifyPolicy);
    server.del('/api/users/:uuid/policies/:policy', auth.requireAuth, auth.requireRole('operators'), resume, bodyParser, users.deletePolicy);

    server.get('/api/users/:account/:uuid/keys', auth.requireAuth, users.listKeys);
    server.post('/api/users/:account/:uuid/keys', auth.requireAuth, auth.requireRole('operators'), resume, bodyParser, users.addKey);
    server.del('/api/users/:account/:uuid/keys/:fingerprint', auth.requireAuth, auth.requireRole('operators'), users.deleteKey);

    server.get('/api/users/:uuid/keys', auth.requireAuth, users.listKeys);
    server.post('/api/users/:uuid/keys', auth.requireAuth, auth.requireRole('operators'), resume, bodyParser, users.addKey);
    server.del('/api/users/:uuid/keys/:fingerprint', auth.requireAuth, auth.requireRole('operators'), users.deleteKey);

    server.get('/api/users/:uuid/2fa', auth.requireAuth, users.getTwoFactorAuthStatus);

    server.patch('/api/users/:uuid/2fa', auth.requireAuth, auth.requireRole('operators'), resume, bodyParser, users.updateTwoFactorAuthStatus);

    server.get('/api/users/:uuid', auth.requireAuth, users.get);
    server.get('/api/users/:account/:uuid', auth.requireAuth, users.get);
    ...
    var manta = require('./api/manta').mount(server, [auth.requireAuth]);
    ...
    server.get('/libs.js', assets.file(this, 'libs.js'));
    server.get('/test.*', restify.serveStatic({directory: './www', 'default': 'index.html'}));
