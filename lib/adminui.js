...
ADMINUI.prototype.createServer = function() {
    ...
    server.put("/_/vms/:uuid", auth.requireAuth, resume, bodyParser, vms.update);
    server.patch("/_/vms/:uuid", auth.requireAuth, resume, bodyParser, vms.update);
    server.put("/_/vms/:uuid/tags", auth.requireAuth, resume, bodyParser, vms.updateTags);
    server.put("/_/vms/:uuid/customer_metadata", auth.requireAuth, resume, bodyParser, vms.updateCustomerMetadata);
    ...
    server.post('/_/packages', auth.requireAuth,
        resume,
        bodyParser,
        packages.add);
    ...
    server.patch('/_/servers/:uuid', auth.requireAuth,
        resume,
        bodyParser,
        servers.update);
    server.del('/_/servers/:uuid', auth.requireAuth, servers.del);

    server.post('/_/servers/:uuid', auth.requireAuth,
        resume,
        bodyParser,
    ...
    server.post('/_/notes/:item_uuid', auth.requireAuth, resume, bodyParser, notes.create);
    server.put('/_/notes/:item_uuid/:uuid', auth.requireAuth, resume, bodyParser, notes.update);


    var networks = require('./networks');
    server.get('/_/networks', auth.requireAuth, networks.list);
    server.get('/_/networks/:uuid', auth.requireAuth, networks.get);
    server.post('/_/networks', auth.requireAuth,
        resume,
        bodyParser,
        networks.create);
    server.get('/_/networks/:uuid/ips', auth.requireAuth, networks.listIPs);
    server.del('/_/networks/:uuid', auth.requireAuth, networks.deleteNetwork);
    ...
    server.post('/_/network_pools', auth.requireAuth, resume, bodyParser, networkPools.createNetworkPool);
    server.put('/_/network_pools/:uuid', auth.requireAuth, resume, bodyParser, networkPools.updateNetworkPool);
    server.del('/_/network_pools/:uuid', auth.requireAuth, networkPools.deleteNetworkPool);

    var users = require('./users');
    server.get('/_/users', auth.requireAuth, users.list);
    server.post('/_/users', auth.requireAuth,
        resume,
        bodyParser,
        users.create);
    ...
    server.patch('/_/users/:uuid', auth.requireAuth, resume, bodyParser, users.update);
    ...
    server.post('/_/users/:uuid/limits', auth.requireAuth, resume, users.addLimit);
    server.put('/_/users/:uuid/limits/:datacenter', auth.requireAuth, resume, users.updateLimit);
    server.del('/_/users/:uuid/limits/:datacenter', auth.requireAuth, users.deleteLimit);
    ...
    server.get('/_/amon/alarms/:user/:uuid', auth.requireAuth, amon.getAlarm);
    server.post('/_/amon/alarms/:user/:uuid', auth.requireAuth, resume, bodyParser, amon.alarmAction);


    server.get(/\/fishbulb\/?.*/, restify.serveStatic({
        'directory': './public/',
        'default': 'fishbulb/index.htm'
    }));
