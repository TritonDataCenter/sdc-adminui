...
ADMINUI.prototype.createServer = function() {
    ...
    server.get(
        {name: 'listDatacenters', path: '/_/datacenters'},
        auth.requireAuth,
        function listDatacenters(req, res, next) {
            var dcs = Object.keys(adminui.config.datacenters);
            res.send(dcs);
            return next();
        }
    );
    ...
    server.get('/_/amon/probes/:user/:uuid', auth.requireAuth, amon.getProbe);
